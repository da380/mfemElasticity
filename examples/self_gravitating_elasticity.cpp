// ============================================================================
// self_gravitating_elasticity.cpp
//
// Driver for SelfGravitatingElasticProblem (mfemElasticity/self_gravitating.hpp):
...
//    ./self_gravitating_elasticity -m ../data/coupled_poisson.msh -o 1 -s 2
//    ./self_gravitating_elasticity -o 2 -s 2 -diag
//
...
int main(int argc, char* argv[]) {
  ...
  auto rheology =
      GeneralisedMaxwellRheology::Elastic(dim, kappa_coeff, mu_coeff);
  ...
  if (schur && minres) {
    GridFunction du(minres->u), dphi(minres->phi);
    du -= schur->u;
    dphi -= schur->phi;
    std::cout << "MINRES vs Schur: ||du||/||u|| = "
              << L2Norm(du) / L2Norm(schur->u)
              << ", ||dphi||/||phi|| = " << L2Norm(dphi) / L2Norm(schur->phi)
              << "\n";
  ...
  if (paraview) {
    ParaViewDataCollection dc("self_gravitating", &body);
