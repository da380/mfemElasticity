...
#include "uniform_sphere.hpp"  // Contains helper functions like MeshCentroid
...
int main(int argc, char *argv[]) {
  ...
  // Mark domain attribute 1 as active
  auto dom_marker = Array<int>(mesh.attributes.Max());
  dom_marker = 0;     // Set all to inactive
  dom_marker[0] = 1;  // Set attribute 1 to active
  ...
  // (These helpers seem to be for validation but aren't used in the main logic)
  ...
  // Create a DiffeomorphismCoefficient. This object wraps the transformation
  // 'qv' and provides methods to compute its inverse and Jacobian, which are
  // needed by the transformed integrator.
  ...
  // Jacobian of the transformation 'xi'.
  ...
  auto h = TransformedFunctionCoefficient(
      xi, std::function(dim == 2 ? exact_solution2 : exact_solution3));
  ...
  // Project the transformation 'xi' (which computes q(x)) onto 'y'
