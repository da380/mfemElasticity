...
int main(int argc, char *argv[]) {
  ...
  auto xi =
      VectorFunctionCoefficient(dim, [r1, r2](const Vector &x, Vector &y) {
        auto r = x.Norml2();
        y = x;
        if (r > r1) {
          auto f = 1.01;
          auto fac = (f * r2 - r1) / (f * r2 - r);
          y *= fac;
        }
      });
  ...
  a.AddDomainIntegrator(new TransformedLaplaceIntegrator(xi));
  a.Assemble();
  ...
  auto sMesh = SubMesh::CreateFromDomain(mesh, Array<int>{1, 0});
  auto sFes = FiniteElementSpace(&sMesh, &fec);
