...
MixedBilinearFormSubMesh::MixedBilinearFormSubMesh(FiniteElementSpace *tr_fes,
  ...
  if (extended_trial) {
    SubMeshUtils::BuildVdofToVdofMap(*sub_fes, *trial_fes, from,
                                     parent_element_ids, *vdof_to_vdof_map);
  } else {
    SubMeshUtils::BuildVdofToVdofMap(*sub_fes, *test_fes, from,
                                     parent_element_ids, *vdof_to_vdof_map);
  }
}
    ...
    for (int i = 0; i < sub_fes->GetNE(); i++) {
      ...
      if (extended_trial) {
        ...
        for (int k = 0; k < domain_integs.Size(); k++) {
          if (domain_integs_marker[k] == NULL ||
            ...
            domain_integs[k]->AssembleElementMatrix2(
                *sub_fes->GetFE(i), *test_fes->GetFE(i), *eltrans, elemmat);
            elmat += elemmat;
          }
        }
        ...
        for (int l = 0; l < trial_vdofs.Size(); l++) {
          ...
          real_t s2 = 1.0;
          int parent_vdof =
              FiniteElementSpace::DecodeDof((*vdof_to_vdof_map)[sub_vdof], s2);
        ...
        for (int k = 0; k < domain_integs.Size(); k++) {
          if (domain_integs_marker[k] == NULL ||
              (*(domain_integs_marker[k]))[elem_attr - 1] == 1) {
            domain_integs[k]->AssembleElementMatrix2(
                *trial_fes->GetFE(i), *sub_fes->GetFE(i), *eltrans, elemmat);
            elmat += elemmat;
          }
        }
        TransformDual(ran_dof_trans, dom_dof_trans, elmat);
...
ParMixedBilinearFormSubMesh::ParMixedBilinearFormSubMesh(
  ...
  if (extended_trial) {
    SubMeshUtils::BuildVdofToVdofMap(*sub_pfes, *trial_pfes, from,
                                     parent_element_ids, *vdof_to_vdof_map);
  } else {
    SubMeshUtils::BuildVdofToVdofMap(*sub_pfes, *test_pfes, from,
                                     parent_element_ids, *vdof_to_vdof_map);
  }
}
      ...
      if (extended_trial) {
        ...
        for (int k = 0; k < domain_integs.Size(); k++) {
          if (domain_integs_marker[k] == NULL ||
              (*(domain_integs_marker[k]))[elem_attr - 1] == 1) {
            domain_integs[k]->AssembleElementMatrix2(
                *sub_pfes->GetFE(i), *test_pfes->GetFE(i), *eltrans, elemmat);
            elmat += elemmat;
          }
        }
        TransformDual(ran_dof_trans, dom_dof_trans, elmat);
        ...
        for (int k = 0; k < domain_integs.Size(); k++) {
          if (domain_integs_marker[k] == NULL ||
              (*(domain_integs_marker[k]))[elem_attr - 1] == 1) {
            domain_integs[k]->AssembleElementMatrix2(
                *trial_pfes->GetFE(i), *sub_pfes->GetFE(i), *eltrans, elemmat);
            elmat += elemmat;
          }
        }
        TransformDual(ran_dof_trans, dom_dof_trans, elmat);
