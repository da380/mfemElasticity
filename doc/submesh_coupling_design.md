...
*30 August 2026. Companion to `status_and_roadmap.md` §3. Written against MFEM 4.9.1 (`ec39b3509c`). Implemented 1–2 September 2026; see the status notes under §2.4 and §7.*
...
**Status (2 Sep 2026): Phases 1–3 complete.** Layer B is `SubMeshMixedBilinearForm` / `ParSubMeshMixedBilinearForm` in `submesh.hpp/cpp`. Implementation notes, where they differ from §3:

- Instead of the CRTP mixin of §3.2, the shared state is a small `detail::SubMeshFormSetup` (shadow space, injection, orientation) and the shared work is a free function `detail::AssembleOnSubMesh(form, setup, skip_zeros)` that builds the borrowing helper, assembles, finalizes and re-indexes; only the two five-line `Assemble` bodies touch the protected `mat` (and `p_mat` in parallel). `Assemble()` *replaces* the matrix rather than accumulating, and resets `mat_e`/`p_mat`.
- The 4.9 borrowing constructor also omits `boundary_face_integs`; they are pushed into the helper through `GetBFBFI()`. Interior-face integrators are refused (`MFEM_VERIFY`), as planned in §5. Only `AssemblyLevel::LEGACY` is accepted.
- Markers are sized against the *submesh's* `attributes.Max()` / `bdr_attributes.Max()`, as MFEM's own `Assemble` requires; in parallel these are the globally reduced ParSubMesh attribute lists, so ranks without submesh elements pass the marker-size check.
- Tests: `tests/TestSubMeshMixedBilinearForm.cpp` (gtest, 264 cases: dim × element type × order 1–3 × orientation × three submesh-side families — scalar H1 with its own collection, vector H1 byVDIM, L2 of order − 1). References are (i) MFEM's `MixedBilinearForm` on the parent with attribute-restricted domain *and* boundary integrators, re-indexed with the explicit injection — exact to 1e-12 relative; (ii) `SubMesh::Transfer` on random vectors against a plain submesh form, for boundary integrators on the *cut* (test 4 proper — the ex11/ex12 case) and for `From::Boundary` submeshes (test 5); (iii) an analytic surface integral through `BoundaryLFIntegrator` on the parent. Test 3 (entrywise agreement with the old `MixedBilinearFormSubMesh`, domain integrators, all 72 configurations) was run once before deletion and passed; it is not kept. `tests/TestSubMeshMixedBilinearFormPar.cpp` (MPI, 1/2/4 ranks, 624 checks each) compares `gᵀ A f` from `ParallelAssemble()` and from `FormRectangularSystemMatrix()` against the serial class on the same mesh, with empty-rank partitions and cut/exterior boundary integrators (tests 7–8).
- Test 6: `ex7`/`ex8` (serial) and `ex7p`/`ex8p` (2 ranks, `-m ex7.msh`) print L2 errors identical to the pre-migration baselines. `ex9`–`ex12` (+p) compile with the new classes but were **not run**: their meshes (`ex9_2d.msh`, `ex10_2d.msh`, `mesh/ex11_2d.msh`) are products of the shelved meshing tools and are not in the tree. When they are next run, `ex11`/`ex12` will for the first time include the interface boundary integrators (§0 item 1), so their results will differ from anything produced on `develop` before this change.
- One MFEM limitation met in testing: `GradientIntegrator::AssembleElementMatrix2` sizes the Jacobian adjugate by the reference dimension, so it corrupts memory on manifold (boundary-submesh) elements. A scalar/vector coupling *on* a boundary submesh therefore needs its own integrator; the vector family is skipped there in the tests.
- Not done, still open: `NCSubMesh`/nonconforming parents (refused by `MFEM_VERIFY`), H(div)/H(curl) shadows, partial assembly, `Update()` after refinement — all per §5. `ex5`/`ex6` now use `SubMeshDofInjection(...).NewSparseMatrix()`.
