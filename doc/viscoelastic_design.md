...
5. **Ordering assumptions.** The pointwise loops assume `Ordering::byNODES` on the L2 tensor space (component `c` occupies `[c·nd, (c+1)·nd)`); assert it in the `Field` constructor. The trace-free component convention is `TraceFreeSymmetricMatrixIndex` (`SymmetricComponentOrder::TraceFreeDropped` in `index.hpp`); the anisotropic plan's tensor ordering is chosen to match.
...
- **`p − 1` resolves `dev ε(u)` only on simplices.** On quads/hexes the gradient of a `Q_p` field has `Q_{p,p−1}` components, so the L2 order must be `p` for the map to be exact and for the effective-modulus elimination to be consistent (the long-time limit of a clamped body missed the `μ_∞` elastic solution by 0.6 % with `p − 1` on quads). The default `internal_order < 0` now picks `p − 1` on simplex meshes and `p` when the mesh has any tensor-product/prism/pyramid geometry (reduced across ranks in parallel).
- The consistent-displacement cache: after a trapezoid or implicit step `u` already satisfies `K_U u = f + Σ Bᵀ 2μ_k m_k^{n+1}`, so the next trapezoid step's `d^n` and a driver's `SolveElastic` reuse it; the cache key is `(m, t)` compared exactly (with an all-reduce in parallel). Exponential Euler and explicit stages invalidate it. Switching between the unrelaxed and an effective operator is lazy and costs one reassembly, so mixing schemes works but is not free.
