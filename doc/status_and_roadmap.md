...
| `spherical_harmonics.hpp/cpp` — `SurfaceHarmonics`, `HarmonicExpansionCoefficient`, `BoundaryHarmonicCoefficients` (added 3 Sep 2026, follow-up F3) | Real orthonormal harmonics with an index map; synthesis (surface field or interior harmonic); analysis of a field on any spherical boundary (scalar or radial component), the boundary counterpart of the DtN coefficient matrix; load vectors from coefficients | Done, serial + parallel | gtest (incl. against `PoissonDtNOperator`) + MPI 1/2/4 |
| `solvers.hpp/cpp` — `RigidTranslation`, `RigidRotation`, `RigidBodySolver` (since 3 Sep 2026: `NullSpaceProjector`, `ProjectedSolver`, `MakeRigidModeProjector()`; follow-up F1) | Wraps a solver with projections orthogonal to the six (three in 2D) rigid modes | Done, serial + parallel | None |
...
| `elastogravity`/`_p` (deleted 3 Sep 2026, see §7.1) | **The target problem**: static self-gravitating elastic sphere under a surface load, block Gauss–Seidel with under-relaxation ω = 0.5 | **not built on `develop`** — the targets exist only in `developZY4`'s `examples/CMakeLists.txt`; needs `ex5_2d.msh`, which is not in `data/` |
...
- Uncommitted: `examples/elastic.hpp`, `viscoelasticity.cpp` (new), rewritten `quasi_static_elasticity.cpp`, `ex1.cpp` edits. Worth committing as a WIP so that the abstraction is visible.
...
1. **`viscoelasticity` aborts on the first time step** (`star.mesh`, `-p 0`): "elastic solve failed". Cause: `LinearQuasiStaticTractionProblem` sets `iterative_mode = true` on the rigid-body wrapper, so the second solve warm-starts from the converged displacement. MFEM's `CGSolver` measures the *relative* tolerance against the **initial** residual, which is already ~1e-12·‖b‖, so the target becomes ~1e-24 and CG stalls at 10 000 iterations. This is the same pitfall `elastogravity.cpp` works around with an "adaptive abs tol" after iteration 1. The fix is a proper absolute tolerance tied to ‖b‖ (or a residual test relative to ‖b‖), not `iterative_mode = false`. Worth solving once, in the library, since warm starts are exactly what time stepping wants.
...
5. gmsh meshes load with "Elements with wrong orientation: 10624/16136 (fixed)"; harmless, but the meshing tools could emit consistently oriented elements. *(Resolved 16 Sep 2026: the meshes are now generated by planetmodel, see `doc/meshing_design.md`, and load with no orientation fixes.)*
...
### 7.1 Cleanup done on 2 September 2026

- Examples pruned to the set that exercises library code and renamed after what they do. Mapping from the old names: `ex2` → `poisson_dtn`, `ex3` → `transformed_diffusion`, `ex8` → `coupled_poisson`, `ex10` → `elastogravity`, `ex11` → `elastogravity_two_layer`, `ex12` → `elastogravity_three_layer` (each with its `_p`). Deleted: `ex1` (superseded by `quasi_static_elasticity`), `ex4` (subset of `ex2`), `ex5`/`ex6` (superseded by `submesh_injection`), `ex7` and `grid_transfer` (Gauss–Seidel variants of `ex8`), `ex9` (fixed-point variant of `ex10`), and `ode`/`wave`/`seismic` (used no library code). Older sections of this document and of the design notes still use the old names.
- Canned meshes committed in `data/` for the examples that need gmsh output (`coupled_poisson.msh`, `elastogravity*_2d.msh`); the unused MFEM stock meshes were removed from `data/`.
- `apps/CMakeFiles`, `texput.log` untracked; `.gitignore` extended; `radial_model.hpp/cpp` (orphan) deleted; README rewritten (the configure scripts it described did not exist).
- `TransformedDiffusionIntegrator` scalar-path Jacobian fixed (§2.4 item 3) and covered by `tests/TestTransformedDiffusionIntegrator.cpp`. The class is kept: it is the seed of a fully referential (arbitrary reference body) formulation of the elastic problem in the sense of Al-Attar & Crawford (2016) and Maitra & Al-Attar (2021), for later.
- Still present, deliberately: the ad-hoc two/three-block rigid-body projectors in `examples/common.hpp` (§2.4 item 4). `elastogravity*` need them as written; `LinearQuasiStaticSelfGravitatingProblem` will not (the Schur complement in u has exactly the rigid null space, so `RigidBodySolver` applies unchanged), and they go when those examples become drivers of the new class. — **Gone, 3 Sep 2026**: `elastogravity(_p).cpp` (the hand-assembled block-MINRES drivers, superseded by `self_gravitating_elasticity(_p)` and `elastogravity_layered(_p)` on the class) and the `TwoBlockRigidBodySolver(Parallel)` classes were deleted; `examples/common.hpp` keeps `Nondimensionalisation` and `Constants` only.
...
3. **Library `elasticity.hpp`** (2 d): promote the `LinearQuasiStaticProblem` design, serial + parallel; generalised null-space projector (list of null vectors, optional mass weighting); tolerance handling. — **Done, 2 Sep 2026** as `rheology.hpp` + `quasi_static_problem.hpp` (`viscoelastic_design.md` §6 status); the generalised null-space projector exists (`NullSpaceProjector` / `ProjectedSolver`, step 4) but `LinearQuasiStaticTractionProblem` still uses the old `RigidBodySolver`; see follow-up F1.
4. **`LinearQuasiStaticSelfGravitatingProblem`** (3–5 d): Π-based coupling blocks, Schur-complement CG as default with MINRES/block-diagonal as an alternative, DtN assembled once, 2D constant-mode handling; reproduce `elastogravity` results; then a Love-number / surface-load benchmark of a PREM-like sphere against your radial codes (this is the first real physics verification of the whole stack and should come before anything below). — **Done, 2 Sep 2026** (`self_gravitating.hpp/cpp`, serial + parallel in one class, derived from `LinearQuasiStaticProblemBase`; tests `TestSelfGravitating.cpp` and `TestSelfGravitatingPar.cpp`; driver `examples/self_gravitating_elasticity(_p).cpp`). Findings that changed the plan:
   - *Default solver is block MINRES*, not the Schur CG: on the 2-D order-2 mesh MINRES takes 0.07 s against 1.1 s for the Schur CG (165 outer × ~140 inner CG iterations), and 0.16 s against 1.5 s in 3-D. Both are kept; the Schur solver is the gauge-clean reference.
   - *The discrete rigid modes are only near-null vectors* of the coupled system (`RigidModeResiduals()`: 6e-4 at order 1, 8e-7 at order 2 in 2-D; 3e-3 / 1e-4 in 3-D). Each solver therefore regularises by projection (`NullSpaceProjector`, `ProjectedOperator`, `ProjectedSolver` in `solvers.hpp`, the generalised projector of §4.3), and the output gauge is fixed explicitly and identically for both: u ⊥ rigid modes (Euclidean), φ solved from u. The MINRES projector uses the coupled null vectors with a proper block-Euclidean Gram–Schmidt, which resolves §2.4 item 4.
   - *Order-1 elements on curved (gmsh order-2) meshes* do not contain the rigid rotations exactly, so at order 1 the class and `elastogravity` differ by ~10% on the example's own problem while at order 2 they agree to five figures (‖u‖, ‖φ‖). Use order ≥ 2 on curved meshes. The same effect makes `LinearQuasiStaticTractionProblem`'s `RigidBodySolver` (CG on A with projected data rather than on P A P) sensitive at order 1; switching it to the `ProjectedOperator` form is a small follow-up.
   - Not yet done from step 4: the PREM-like Love-number benchmark against the radial codes.
5. **Anisotropic integrator + TI coefficient + tests** (2–3 d): independent of 2–4, can run in parallel with them. — **Done, 2 Sep 2026** (`elastic_tensor.hpp`; `anisotropic_elasticity_design.md` §5 status). Not yet used by the elastic/viscoelastic problem layer.
6. **Viscoelastic upgrades** (3 d): ETD2 or implicit stepping; material coefficients exported from the elastic problem; gravity-coupled viscoelastic run (GIA-style relaxation of a uniform sphere, checkable against the radial codes). — **Stepping and material export done, 2 Sep 2026** (`viscoelastic.hpp`: exponential trapezoid, ETD1, BE/SDIRK via `ImplicitSolve`, generalised Maxwell, Galerkin strain map; see `viscoelastic_design.md` §6 status). The gravity-coupled run waits for step 4.
...
### 8.1 Follow-ups agreed on 3 September 2026

- **F1. Retire `RigidBodySolver`** (½ d, serial + MPI). It is the special case of `ProjectedSolver` with a `NullSpaceProjector` holding the six (three in 2-D) rigid modes, and the weaker one: it hands CG the unprojected `A` (only data and solution are projected; see step 4's order-1 note), does not project a warm-start initial guess, and its Gram–Schmidt cannot drop a dependent vector. Rigid-mode construction is also triplicated (the old solver, `LinearQuasiStaticSelfGravitatingProblem::SetupRigidModes`, the block solvers in `examples/common.hpp`). Plan: (1) in `solvers.hpp`, a free function appending the rigid modes of a displacement space to a `NullSpaceProjector` (communicator from the space) and a factory returning a projector holding just those modes; (2) `LinearQuasiStaticTractionProblem` builds its projector with the factory and wraps *both* CG and its preconditioner in `ProjectedSolver`s, as the self-gravitating class does (an unprojected AMG amplifies the round-off null component and CG diverges in parallel); (3) `SetupRigidModes` calls the free function; (4) delete `RigidBodySolver` (the legacy `elastogravity*` drivers keep their own block solvers until they are retired); (5) a direct unit test, serial + MPI: a free bar's singular stiffness through `ProjectedSolver` + CG converges, the solution is orthogonal to every basis vector, and a load with net force or torque gives the same displacement as its projected part; (6) update the mentions of the old class here and in `viscoelastic_design.md`. Expected effects: Cartesian test meshes unchanged to round-off; order-1 results on curved meshes move slightly (and should improve). The mass-weighted gauge of §4.3 (zero net momentum rather than dof-space orthogonality) stays a separate, later option on the projector. — **Done, 3 Sep 2026**: `AddRigidModes()` / `MakeRigidModeProjector()` in `solvers.hpp`; `LinearQuasiStaticTractionProblem` runs CG on `P A P` with `P M P` (the base class now separates `SetupDefaultPreconditioner()` and `SetupCG()`, which the self-gravitating Schur path shares); `SetupRigidModes` uses the factory; `RigidBodySolver` deleted (and the unmaintained `apps/` directory, its only other user, removed); tests `TestSolvers.cpp` (gtest) and `TestSolversPar.cpp` (MPI 1/2/4): basis size and orthonormality, exact null vectors on Cartesian meshes, convergence with net force and torque, orthogonality of the solution, projected residual, equality with the projected load and with the serial solve, warm start with a rigid component.
- **F2. Composite rheologies**: `composite_rheology_design.md` (Phase 1 rheology-layer only, before the Love-number benchmark; Phase 2 region-restricted internal variables, after it). — **Phase 1 done, 3 Sep 2026** (§5 answered, status in §6 of the design doc): `CompositeRheology` by element attribute, marker-restricted stiffness integrators, region-labelled output fields; tests serial + MPI. **Phase 2 done too, 3 Sep 2026** (§4 of the design doc): each branch's internal variable is stored and evolved on its region's nodes only, with the coupling rows restricted to them; state and per-step work scale with the region.
- **F4. Mass-weighted rigid gauge** (§4.3's second projection) — **Done, 3 Sep 2026**. `ProjectedSolver::SetGauge(M)`: after the (Euclidean-projected) solve the null-space component of the solution is re-gauged so that `n_iᵀ M x = 0` for every basis vector with nonzero `M`-norm (zero net momentum and angular momentum when `M` is the ρ-weighted vector mass matrix; vectors of zero `M`-norm, the 2-D constant potential, keep the Euclidean condition). The inner solve is untouched: an `M`-weighted projector would make `P A P` non-symmetric, and the gauge is only a choice of representative. `LinearQuasiStaticTractionProblem::SetMassWeightedGauge(rho)` / `SetEuclideanGauge()`; `LinearQuasiStaticSelfGravitatingProblem::SetMassWeightedGauge(on)` with the body's density, for both solvers — under this gauge the block-MINRES path solves the potential from the gauged displacement (the rigid shift moves the body's potential with it), and the two solvers return the same `(u, φ)` directly. Tests in `TestSolvers(Par)`, `TestQuasiStaticProblem`, `TestSelfGravitating`: zero moments, strains unchanged, difference from the Euclidean representative a rigid motion, Schur = MINRES.
- **F3. Love-number benchmark** (step 4's open item): needs the radial-code numbers; a `love_numbers` driver with spherical-harmonic surface extraction to write. — **Driver and extraction done, 3 Sep 2026**; the PREM comparison waits for the numbers. `spherical_harmonics.hpp`: `SurfaceHarmonics` (real orthonormal harmonics, Dahlen & Tromp in 3-D, Fourier in 2-D, index `l² + l + m`), `HarmonicExpansionCoefficient` (synthesis: surface field, or interior harmonic `(r/R)^l` for tidal potentials), `BoundaryHarmonicCoefficients` (analysis of a scalar field or the radial component of a vector field on any spherical boundary, serial and MPI; the boundary counterpart of the DtN operator's coefficient matrix, against which it is tested; `LoadVector()` is the transpose direction). `LinearQuasiStaticSelfGravitatingProblem::PotentialSpaceOnBody()` and `SolveLoadPotential()` (the loads' potential with the body rigid) added. `examples/love_numbers.cpp`: unit-coefficient load or tidal potential at each degree, one solve each; h, k, h', k' from the surface coefficients, with the load's own potential taken from the rigid-body solve on the same mesh (subtracting the exact value instead leaves the direct potential's discretisation error in k', a 30% effect on the coarse 3-D mesh). Check against the incompressible homogeneous sphere (Wu & Peltier 1982) on `coupled_poisson.msh`, order 2, κ/μ = 200: h, k, h', k' within 2–3% at degree 2 and 5–15% at degree 4, the coarse mesh's own error (the printed direct-potential ratio and the spurious-coefficient level say as much); on a finer ball-in-ball mesh (`meshes/ball_with_buffer.py` with sizes 0.12/0.4, 2 min serial at order 2) all four agree to 0.1–0.7% at degrees 2–4, the remainder being the finite bulk modulus; that is the physics verification of the coupled solver. The PREM run waits for the radial numbers. Tests `TestSphericalHarmonics(.cpp/Par.cpp)`.
