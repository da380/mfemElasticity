...
4. **`SelfGravitatingElasticProblem`** (3–5 d): Π-based coupling blocks, Schur-complement CG as default with MINRES/block-diagonal as an alternative, DtN assembled once, 2D constant-mode handling; reproduce `elastogravity` results; then a Love-number / surface-load benchmark of a PREM-like sphere against your radial codes (this is the first real physics verification of the whole stack and should come before anything below). — **Done, 2 Sep 2026** (`self_gravitating.hpp/cpp`, serial + parallel in one class, derived from `ElasticProblemBase`; tests `TestSelfGravitating.cpp` and `TestSelfGravitatingPar.cpp`; driver `examples/self_gravitating_elasticity(_p).cpp`). Findings that changed the plan:
