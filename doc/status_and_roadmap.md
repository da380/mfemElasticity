...
- Still present, deliberately: the ad-hoc two/three-block rigid-body projectors in `examples/common.hpp` (§2.4 item 4). `elastogravity*` need them as written; `SelfGravitatingElasticProblem` will not (the Schur complement in u has exactly the rigid null space, so `RigidBodySolver` applies unchanged), and they go when those examples become drivers of the new class.
...
3. **Library `elasticity.hpp`** (2 d): promote the `QuasiStaticLinearElasticProblem` design, serial + parallel; generalised null-space projector (list of null vectors, optional mass weighting); tolerance handling. — **Done, 2 Sep 2026** as `rheology.hpp` + `elastic_problem.hpp` (`viscoelastic_design.md` §6 status); the generalised null-space projector is still the existing `RigidBodySolver`.
4. **`SelfGravitatingElasticProblem`** (3–5 d): Π-based coupling blocks, Schur-complement CG as default with MINRES/block-diagonal as an alternative, DtN assembled once, 2D constant-mode handling; reproduce `elastogravity` results; then a Love-number / surface-load benchmark of a PREM-like sphere against your radial codes (this is the first real physics verification of the whole stack and should come before anything below). — **Done, 2 Sep 2026** (`self_gravitating.hpp/cpp`, serial + parallel in one class, derived from `LinearElasticProblemBase`; tests `TestSelfGravitating.cpp` and `TestSelfGravitatingPar.cpp`; driver `examples/self_gravitating_elasticity(_p).cpp`). Findings that changed the plan:
