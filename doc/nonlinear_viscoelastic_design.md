...
- `LinearQuasiStaticProblem` (interface; `QuasiStaticLinearElasticProblem` until 3 Sep 2026) — the elastic part stays linear and this is what the future nonlinear-elastic interface will contrast with (`NonlinearQuasiStaticProblem`, with residual and tangent, later).
- The linear base is `LinearQuasiStaticProblemBase` (`ElasticProblemBase` until 2 Sep 2026, `LinearElasticProblemBase` until 3 Sep 2026), freeing the plain name for the base of the nonlinear family when it arrives. The concrete problems carry the family prefix since 3 Sep 2026: `LinearQuasiStaticTractionProblem`, `LinearQuasiStaticClampedProblem`, `LinearQuasiStaticSelfGravitatingProblem` (`TractionProblem`, `ClampedProblem`, `SelfGravitatingElasticProblem` before; verbose, but every class of the family says what it is, and the nonlinear family will read the same way).
- Renames of 3 Sep 2026: "elastic" left the problem-layer names because the layer serves elastic and viscoelastic bodies alike — the rheology decides, the problem assembles its (effective) elastic stiffness and solves the equilibrium at time t, and the viscoelastic operator evolves the memory around it. Files: `elastic_problem.hpp/cpp` → `quasi_static_problem.hpp/cpp`, `TestElasticProblem(Par).cpp` → `TestQuasiStaticProblem(Par).cpp`, `ElasticTestCommon.hpp` → `QuasiStaticTestCommon.hpp`.
...
7. **Self-gravitating**: a Maxwell-with-power-law mantle under the surface load runs through `LinearQuasiStaticSelfGravitatingProblem` unchanged (smoke test; physics with the Love-number machinery later).
...
6. **Names** (§3.2–3.3): `IsotropicMaxwellRheology` / `AnisotropicMaxwellRheology`; `LinearQuasiStaticProblemBase`; the interface `LinearQuasiStaticProblem` unchanged; `ViscoelasticOperator` unchanged (general). Alternatives welcome — this is the cheapest moment to rename.
...
- Rheology: `MaxwellBranch` / `AnisotropicBranch` carry an optional law; `Rheology::Law(k)`, `IsLinear()`, `UnrelaxedModulus()` (for the nodal stress of anisotropic bodies); the `Maxwell` and `DeviatoricMaxwell` factories take a law. Renames done: `IsotropicMaxwellRheology`, `LinearQuasiStaticProblemBase`.
