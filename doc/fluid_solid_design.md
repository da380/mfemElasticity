...
- **Tests** `TestSelfGravitatingFluid` (gtest) and `TestSelfGravitatingFluidPar` (MPI) on the three-layer meshes (`data/elastogravity_three_layer_{2d,3d}.msh`, the 3-D one new: 13.8k order-2 tets, since regenerated by `meshes/layered_earth.py`) and the two-layer disc; test model in `SelfGravitatingTestCommon.hpp`.
