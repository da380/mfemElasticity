...
Part I (§1–§6) is the fluid–solid extension of `SelfGravitatingElasticProblem`. Part II (§7) is the anisotropic extension of `LinearElasticProblemBase` / `ViscoelasticOperator`, which is independent and can be done at any time. §8 lists the decisions I would like confirmed before writing code.
...
The first six lines are what `SelfGravitatingElasticProblem` already assembles (with M_S = M). (F1)–(F3) are new. A is symmetric, and the rigid-mode identity of the main text (eq. 9) is retained.
...
SelfGravitatingElasticProblem(fes_u, fes_phi, rheology, density_S, G, dtn_degree,
...
The problem layer is isotropic by construction in three places: `IsotropicMaxwellRheology` (κ, μ_∞, branches (μ_k, τ_k)); `LinearElasticProblemBase` assembles two `ElasticityIntegrator`s and swaps a *scalar* effective shear modulus; `ViscoelasticOperator` keeps *trace-free* internal variables m_k with force Bᵀ(2μ_k m_k) through the trace-free coupling form B and forms μ_eff = μ_∞ + Σ_k β_k μ_k pointwise.
...
- `LinearElasticProblemBase` stops knowing about κ and μ: it calls `rheology.AddStiffnessIntegrators(*integrators_)` in the constructor and replaces `SetEffectiveShearModulus(i, mu_eff)` by `SetRelaxationWeights(i, betas)` / `ClearRelaxationWeights()`; `SupportsEffectiveShearModulus()` becomes `SupportsRelaxationWeights()`. For the isotropic rheology `SetRelaxationWeights` computes μ_eff = μ_∞ + Σ β_k μ_k as a `SumCoefficient` chain — or, as now, nodally in the viscoelastic operator; the current nodal μ_eff GridFunction is exact at nodes and the chain is exact everywhere, so this is a (tiny) improvement rather than a regression. `SelfGravitatingElasticProblem` needs no change beyond the renamed calls.
...
- **`IsotropicMaxwellRheology : Rheology`** keeps its whole API. **`AnisotropicMaxwellRheology : Rheology`** takes @f$C_\infty@f$ and `AnisotropicBranch{C_k, tau_k}` (Mandel `MatrixCoefficient`s); `Elastic(C)` and `DeviatoricMaxwell(C, tau)` (relax @f$P_{dev} C P_{dev}@f$, keep the complement) factories; `UnrelaxedTensor()`.
- **Problem layer**: `QuasiStaticLinearElasticProblem::Rheology(i)` returns `const Rheology&`; `SupportsEffectiveShearModulus / SetEffectiveShearModulus / ClearEffectiveShearModulus` replaced by `SupportsRelaxationWeights / SetRelaxationWeights(i, beta) / ClearRelaxationWeights`; `LinearElasticProblemBase` takes any `Rheology` and no longer knows about @f$\kappa, \mu@f$ (`CurrentShearModulus()` gone; `Stiffness()` exposes the relaxation state). `SelfGravitatingElasticProblem` takes a `Rheology` too, so a self-gravitating anisotropic body needs no further change.
...
On David's Q9 (cache both stiffness versions, for adjoint runs with many observation-time jumps): the design keeps this local to `LinearElasticProblemBase` (a small `(relaxed?, dt)`-keyed cache of `A_` plus preconditioner); not implemented yet.
