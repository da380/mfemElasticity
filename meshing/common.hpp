...
// Helper function to tag all volumes and surfaces with the 'average radius' in the ascending order.
// First is the list of volume tags, second and third are the prefixes of volume and surface tags, respectively.
...
// Helper function to calculate the mean radius of a surface as an arithmetic average over all nodes.
// Returns a double: first is the tag of the surface to be measured
...
private:
    ...
    double _buffer_ratio;   ///< Radio of the buffer layer depth over the reference radius.
    ...
    std::vector<double> density_list, pWave_list, sWave_list, bulkM_list, shearM_list; ///< Optional property tables.
};
...
private:
    ...
    const std::vector<double>& _r_field;   ///< Reference to the sampled radial field.
};
...
// Helper function to perturb radially all nodes in a mesh with the scheme RadialMapping.
// mapping: a specific RadialMapping class
