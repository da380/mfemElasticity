...
try {
    ...
    for (int i=1; i<argc; ++i) {
        ...
        else if (arg == "-buf" && i+1<argc) buffer_ratio = std::stod(argv[++i]);
        else if (arg == "-alg" && i+1<argc) alg3d = std::stoi(argv[++i]);
        else if (arg == "-alg2d" && i+1<argc) alg2d = std::stoi(argv[++i]);
        else if (arg == "-il" && i+1<argc) ignored_layers = std::stoi(argv[++i]);
        ...
        else if (arg == "-decay_thickness" && i+1 < argc) decay_thickness = std::stod(argv[++i]);
    }
    ...
    for (std::size_t i = 0; i < radii_ext.size() - 1; ++i)
        tools.emplace_back(3, gmsh::model::occ::addSphere(0., 0., 0., radii_ext[i]));
    gmsh::model::occ::synchronize();

    std::vector<std::pair<int,int>> outDimTags;
    std::vector<std::vector<std::pair<int,int>>> outDimTagsMap;
    gmsh::model::occ::fragment(objects, tools, outDimTags, outDimTagsMap, -1, true, true);
    ...
    for (const auto &p : outDimTags) {
        if (p.first == 3) volTags.push_back(p.second);
        else if (p.first == 2) surfTags.push_back(p.second);
    }
    std::sort(surfTags.begin(), surfTags.end());
    surfTags.erase(std::unique(surfTags.begin(), surfTags.end()), surfTags.end());
